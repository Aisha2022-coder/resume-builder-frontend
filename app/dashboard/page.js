...
export default function Dashboard() {
  ...
  const [resumeData2, setResumeData2] = useState({
    ...
    professionalSummary: "A highly skilled and motivated Software Engineer with experience in developing and maintaining web applications using React and Node.js. Passionate about building efficient and scalable solutions.",
    workExperience: [
      {
    ...
    achievements: ["Employee of the Month - XYZ Corp", "Developed an AI-powered tool"],
    certifications: [
      {
  ...
  const handleViewResume = () => {
    if (!isFormSubmitted) {
      toast.error("Please submit at least one section before previewing the resume.");
      return;
    }
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-100 p-8">
        ...
        <div className="bg-white p-6 mt-6 rounded-lg shadow-lg">
          <h2 className="flex justify-center text-2xl font-bold mb-4">Selected Resume Template</h2>
          {getSelectedTemplate()}
        </div>
      </div>
      <div className="bg-white p-6 mt-6 rounded-lg shadow-lg">
      ...
      {isLoggedIn && openSection === "personalInfo" && (
        <PersonalInfoForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}

      {isLoggedIn && openSection === "workExperience" && (
        <WorkExperienceForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}

      {isLoggedIn && openSection === "education" && (
        <EducationForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}

      {isLoggedIn && openSection === "skills" && (
        <SkillsForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}

      {isLoggedIn && openSection === "projects" && (
        <ProjectsForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}

      {isLoggedIn && openSection === "achievements" && (
        <AchievementsForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}

      {isLoggedIn && openSection === "certificates" && (
        <CertificationsForm userID={userID} setIsFormSubmitted={setIsFormSubmitted} closeModal={() => setOpenSection(null)} />
      )}
